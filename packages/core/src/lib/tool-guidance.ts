...
export const MEMORY_GUIDANCE =
  ...
  "memories; if the profile is empty, say so in plain text. If a write returns " +
  "staged:true, tell the user it is pending approval — do not claim it is saved.";
...
const GUIDANCE_BY_KEY: Record<
  ToolGuidanceKey,
  { toolNames: readonly string[]; text: string }
> = {
  memory: { toolNames: ["memory"], text: MEMORY_GUIDANCE },
  skills: {
...
export function buildToolGuidance(
  ...
  for (const key of GUIDANCE_ORDER) {
    ...
    const { toolNames: names, text } = GUIDANCE_BY_KEY[key];
